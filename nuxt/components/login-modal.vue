<template>
  <div v-if="isOpen" class="modal-container">
    <div class="fixed top-0 left-0 w-full h-full bg-black opacity-75 z-10"></div>
    <div
      ...
      <section class="mt-6 mb-3 cursor-default font-semibold text-gray-800 text-lg flex justify-between">
        ...
        <div class="flex justify-center">
          <button @click="switchToLoginForm('guestLogin')" :class="{
            'cursor-pointer bg-[#5c6f9f] text-white px-4 py-2 rounded-s-lg border-s-2 border-y-2 tracking-wider': activeForm === 'guestLogin',
            'cursor-pointer px-4 py-2 rounded-s-lg border-s-2 border-y-2 tracking-wider text-gray-800': activeForm !== 'guestLogin'
          }">
            Guest
          </button>
          <button @click="switchToLoginForm('adminLogin')" :class="{
            'cursor-pointer bg-[#5c6f9f] text-white px-4 py-2 rounded-e-lg border-e-2 border-y-2  tracking-wider': activeForm === 'adminLogin',
            'cursor-pointer px-4 py-2 rounded-e-lg border-e-2 border-y-2 tracking-wider text-gray-800': activeForm !== 'adminLogin'
          }">
            Admin
          </button>
        </div>
