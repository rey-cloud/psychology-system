  ...
  <div class="container mx-auto p-4">
    <div class="bg-white p-8 border rounded shadow">
      <div class="mb-4 flex items-center justify-between">
        ...
        <nuxt-link to="/users/create" class="text-blue-500"></nuxt-link>
      </div>
      <div>
...
export default {
  ...
  data() {
    return {
      users: [],
      // Add a filter for users based on type
      userTypeFilter: 'admin',
    };
  },
  computed: {
    // Use a computed property to filter users based on type
    filteredUsers() {
      return this.users.filter(user => user.type === this.userTypeFilter);
    },
  },
  mounted() {
    this.getUsers();
